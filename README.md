...
python test_reliability.py     # 13: estimator recovery + methodological traps
```
...
two-way model separates them: the rater variance grows, the residual does
not, and consistency reliability stays put. `simulate.py` demonstrates this
on data built at a fixed reliability, and `test_reliability.py` asserts it.
...
test_reliability.py     13 tests, including two that encode methodological traps
```
